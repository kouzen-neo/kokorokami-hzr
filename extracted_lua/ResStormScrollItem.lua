...
function M:OnClick()
    ...
    if resType == 10 or resType == 11 or resType == 12 or resType == 13 then
         local datas = self:GetResIDs(resType)
        --随机奖励调整为服务器获得
        local ClientLevelDropInfoREQ = {
            levelID = datas
        }
        local buffer = assert(pb.encode('PBClient.ClientLevelDropInfoREQ',ClientLevelDropInfoREQ))
        MgrNet.SendReq(MID.CLIENT_LEVEL_DROP_INFO_REQ,buffer,0,nil,function(buffer, tag)
            local tab = assert(pb.decode('PBClient.ClientLevelDropInfoACK',buffer))
            if tab.errNo ~= 0 then
                print("请求背包数据失败")
                return
            end
        end,function(buffer, tag)
            local tab = assert(pb.decode('PBClient.ClientLevelDropInfoNTF',buffer))
            if tab then
                for index, value in ipairs(tab.Info) do
                    StormViewModel.ResRewardData[value.levelID] = value
                end
