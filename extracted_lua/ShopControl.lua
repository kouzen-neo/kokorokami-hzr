...
require("LocalData/SkinRandomLocalData")
...
function ShopControl.InitShopData()
    ...
    --初始化抽皮肤礼包数据
    ShopControl.InitRandomSkinData()
end
...
---初始化抽皮肤礼包数据
function ShopControl.InitRandomSkinData()
    for i, v in pairs(SkinRandomLocalData.tab) do
        local tStr = string.split(v.goods,',')
        local tData = {
            skinList = {},
            linkId = v.jumpfield,
            actId = v.activeid
        ...
        for i, v in ipairs(tStr) do
            local tGoods = string.split(v,'_')
            tData.skinList[tonumber(tGoods[2])] = tGoods
        end
        table.insert(Shop_RandomSkin, tData)
    end
end
---skinList 皮肤列表
---linkId 跳转ID
function ShopControl.GetRandomSkinData()
    return Shop_RandomSkin
end
