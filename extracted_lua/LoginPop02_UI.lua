...
function M:OnInit()
    ...
    self.Btn_Yinjima().gameObject:SetActive(false);
...
function M:JpLogin(code, request)
    if code == 0 then
        ...
        local errorTab = LanguageerrorLocalData.tab[code]
        if errorTab then
            MgrUI.Pop(UID.PopTip_UI, {MgrLanguageData.GetLanguageByKey(errorTab[2]), 1}, true)
        else
            UnityEngine.DebugEx.LogError(string.format("JpLogin failed. code: %s, request: %s", code, request))
        end
    end
end
