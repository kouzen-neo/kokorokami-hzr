...
function M:ListCellItem(trans,idx)
    trans:GetComponent("UITemplate"):SetData({self.FriendList[idx],self})
...
function M:ReloadListData(offset)
    ---设置Loop数量
    self.ListScroll01().totalCount = #self.FriendList
    if offset then
    ...
    ---判断有无数据，无数据显示无数据图片
    self.Img_ListNoData().gameObject:SetActive(#self.FriendList == 0)
    ---有好友数据就展示删除好友按钮
    self.Tog_Delete01().gameObject:SetActive(#self.FriendList ~= 0)
    self.Img_NoDelete().gameObject:SetActive(#self.FriendList == 0)
    ---刷新好友币
...
function M:FriendSetRemarkNTF(buffer,tag)
    ...
    self.RemarkPanel().gameObject:SetActive(false)
    ---更新数据
    for i,friend in pairs(self.FriendList) do
