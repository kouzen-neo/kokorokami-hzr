...
function M:OnUpdateUI(pData)
    ...
    ActivityControl.GetCurActivityByID(self.ActiveID)
...
function M:ReloadData()
    self.mActivityData = LimitSignControl.GetLimitSignData()
...
function M:GetActivityData()
    ...
    for i, v in ipairs(SignineventLocalData.tab) do
        if tostring(LimitSignControl.GetActivityInfo().version) == v.group then
            table.insert(tab,v)
        end
    end
    return tab
...
--设置活动时间
function M:SetActivityTime(_id)
...
function M:PointCell(trans,idx)
    ...
    if day <= LimitSignControl.GetSignDay() then--已经签到
        Signitem_Light.gameObject:SetActive(false)
        ...
        if LimitSignControl.GetIsSignDay() and day == LimitSignControl.GetSignDay() + 1 then
...
function M:ClientLimitSignReq(_id,callback)
    ...
    {
        id = _id,
        activityId = LimitSignControl.ReturnActivityID()
    }
    ---序列化
...
function M:ActivityRewardSendNtf(buffer, tag)
    ...
    LimitSignControl.SetInitData(MgrNet.GetServerTime(),LimitSignControl.GetSignDay() + 1)
    --self:ScrollToIndex()
