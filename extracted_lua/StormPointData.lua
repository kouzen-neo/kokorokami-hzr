...
function StormPointData:PushConfig(config)
    ...
    if ChapterLocalData.tab[self.scrollID] == nil then
        print(self.scrollID.."is nil")
    end
...
function StormPointData:GetRewards()
    ...
    if self.randReward ~= "0" and self.randReward ~= nil then
        --如果是资源副本
        local temp = {}
        if self.scrollType == 10 or self.scrollType == 11 or self.scrollType == 12 or self.scrollType == 13 or self.scrollType == 14 then
            local datas = StormViewModel.ResRewardData[self.id].drop
            for index, value in ipairs(datas) do
                temp[value.dropID] = value
            end
        end
        ...
        for i1, v1 in ipairs(t) do
            if v1 ~= nil and v1 ~= "0" and v1 ~= "" then
                ...
                if t1[1] ~= nil and t1[1] ~= "0" and t1[1] ~= "" then
                    ...
                    --资源副本随机奖励读取服务器数据
                    if self.scrollType == 10 or self.scrollType == 11 or self.scrollType == 12 or self.scrollType == 13 or self.scrollType == 14 then
                        rRs = temp[tonumber(t1[1])].dropGoods
                        power = temp[tonumber(t1[1])].num
                    else
                        rRs = DropLocalData.tab[tonumber(t1[1])][4]
                    end
                    if rRs ~= nil and rRs ~= "0" and rRs ~= "" then
                        ...
                        for i2, v2 in pairs(t3) do
                            ...
                            if isGet ~= true then
                                ...
                                data.count = tonumber(v2Self[3]) * power
                                data.isRec = false
                                data.idx = #arr + 1
                                arr[#arr + 1] = data
                            end
