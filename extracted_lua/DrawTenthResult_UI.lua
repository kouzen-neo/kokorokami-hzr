...
function M:OnInit()
    ...
    Event.Add("RefreshCoinCount",Handle(self,self.ChangeWallet))
    ---卡池数据
    ...
    if NoviceViewModel.Noviceing == true then
        ...
        ---货币列表
        self.CoinList = {}
        table.insert(self.CoinList,{ item = "1_100008", link = "tz_sc_cz_01" })
        table.insert(self.CoinList,{ item = "1_100000", link = "dh_sc_mfzs" })
        if self.poolData.singleDrawPrice == self.poolData.tenDrawPrice then
            table.insert(self.CoinList,{ item = self.poolData.singleDrawPrice })
        else
            table.insert(self.CoinList,{ item = self.poolData.tenDrawPrice })
        end
        table.insert(self.CoinList,{ item = self.poolData.item })
        self:ShowCoin()
    end
end
...
function M:ShowCoin()
    for i, v in ipairs(self.CoinList) do
        local tCoinObj = GameObject.Instantiate(self.Btn_Coin().gameObject, self.Huobi().transform, false)
        
        ---更新货币信息
        self.CoinData[i] = self:CoinCell(tCoinObj.gameObject, v)
    end
    self.Btn_Coin().gameObject:SetActive(false)
    ---刷新货币
    self:ChangeWallet()
...
function M:CoinCell(_obj,_data)
    ...
    local tCoin = string.split(_data.item,'_')
    if _data.link then
        tAdd.gameObject:SetActive(true)
        UIEvent.LuaClick(_obj, function()
            MgrLink.LinkStart(_data.link)
        end)
    else
...
function M:ChangeWallet()
    for i, v in ipairs(self.CoinData) do
...
--- 设置货币栏显隐，不会触发货币刷新
function M:SetCoinActive(bActive)
    self.Huobi().gameObject:SetActive(bActive)
end
