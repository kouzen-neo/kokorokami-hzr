...
function ShopViewModel.GetShopHomeList()
    ...
    if data ~= nil then
        for i, v in pairs(data.childShop) do
            if v.Childtype ~= 5 and v.Childtype ~= 1 then
                arr[#arr + 1] = v
            end
        end
        ---排序
...
---@param ntfFun fun()?
function ShopViewModel.SendBuyGoods(goods,ntfFun,needUIPop)
    local buyGoods = {
        goodsID = goods[1],
        buyNum = goods[2],
    }
...
function ShopViewModel.FlyFunBuyGoods(goodsId,callBack)
    ...
    MgrNet.HttpGetOrderGenernal(player.UID,goodsId,function(info)
        ...
        if info.code == 1 then
            ...
            ---拉起支付
            print("拉起支付")
            local price = tostring(tonumber(info.data.cp_price) / 100);
            local price_jpy = tostring(tonumber(info.data.price_jpy) / 100);
            MgrSdk.FlyFunCharge(tostring(player.UID),player.nickname,tostring(player.level),"101","server",
                    ...
                    price,info.data.product_id,info.data.name,info.data.introduce,price_jpy,
                    function(request)
                        ...
                        if data[1] == "0" then
                            ...
                            --local str = MgrLanguageData.GetLanguageByKey("loginpop_ui_tips18")..string.format(":(%d)",data[1])
                            --MgrUI.Pop(UID.PopTip_UI,{str, 2},true)
                            MgrUI.Pop(UID.PopTip_UI, {MgrLanguageData.GetLanguageByKey(LanguageerrorLocalData.tab[tonumber(data[1])][2]) .. " InfoCode:" .. data[1], 1}, true)
                            print(string.format("商品%s,價格%s支付异常：%s",info.data.name,info.data.cp_price,request))
                            --MgrUI.Pop(UID.PopTip_UI,{request,2},true)
                        end
            ...
            Global.PopTip(string.format("%s InfoCode:%s", MgrLanguageData.GetLanguageByKey(CheckTable(LanguageerrorLocalData.tab[info.code])[2]), info.code), nil, true)
            if callBack then
                callBack()
            end
        end
...
function ShopViewModel.RegisterGoodsNtf()
    UnityEngine.DebugEx.LogError("[QuickIOS] ShopViewModel.RegisterGoodsNtf");
    MgrNet.RegisterNTF(MID.CLIENT_HAS_NEW_ORDER_INFO_NTF,function(buffer,tag)
        ...
        UnityEngine.DebugEx.LogError("[QuickIOS] On CLIENT_HAS_NEW_ORDER_INFO_NTF. goods: " .. #info.goods .. ", recharge: " .. info.recharge .. ", allGoods: " .. #info.allGoods)
        ---添加物品
        ...
        ---显示获得
        UnityEngine.DebugEx.LogError("[QuickIOS] On CLIENT_HAS_NEW_ORDER_INFO_NTF. 2")
...
---@return ShopItemData
function ShopViewModel.GetShopDataByID(type1,type2,id)
    local tData = nil
    local tShopList = ShopControl.GetSingleShopData(type1)
    if tShopList then
        local tShopItems = tShopList.childShop[type2].shopItems
        if tShopItems then
            tData = tShopItems[id]
        end
    end
