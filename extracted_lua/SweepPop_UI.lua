...
function M:OnInit()
    ...
    UIEvent.LuaClick(self.Btn_Enter().gameObject,function()
        ...
        if self.curCount == 0 and self.pData.scrollType == 13 then
        ...
        MgrNet.SendReq(MID.CLIENT_SWEEP_LEVEL_STAR_REQ,bytes,0,nil, function(buffer,tag)
            ...
            if recNtf.weekCopyNum then
                StormControl.SaveWeekBossTime(recNtf.weekCopyNum)
            end
            ---检查是否升级
...
function M:TiliSweep()
    ...
    if self.vigor ~= 0 then
        ...
        if self.pData.scrollType == 13 then
            if self.maxCount >= tonumber(SteamLocalData.tab[125000][2]) then
                self.maxCount = tonumber(SteamLocalData.tab[125000][2])
