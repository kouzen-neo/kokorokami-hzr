...
function HomeViewModel.Init()
    ...
    PosterGirlViewModel.CacheVoiceData()
    ---初始化引导
...
---跳转核心提升界面
function HomeViewModel.TurnToCoreUpgrade(index)
    BagViewModel.OpenCoreUpgradeUI(index)
end
---打开困难FB
function HomeViewModel.TurnToHardMode()
    if SysLockControl.CheckSysLock(1108) then
        ...
        StormViewModel.OpenStormPointUI(pData[index], StormViewModel.PointType.main)
    else
