...
BattleViewModel.NormalDuration = 0.3
...
function BattleViewModel.TurnStartCamMove()
    ...
    print("[Battle] BattleViewModel.TurnStartCamMove. round: " .. BattleViewModel.RoundCount .. ", turn: " .. BattleViewModel.TurnCount .. ", idx: " .. BattleViewModel.PhaseIndex)
...
function BattleViewModel.CheckNextPhase()
    ...
    print("[Battle] BattleViewModel.CheckNextPhase. idx: " .. BattleViewModel.PhaseIndex .. ", checkPt: " .. info.checkPt)
...
function BattleViewModel.PlayExs(exList, callback)
    ...
    for key, value in pairs(exTab) do
        ...
        if SettingViewModel.GetExAnim() == 1 and einfo ~= nil then
...
function BattleViewModel.handleSpineEvent(aniName, eName, gameid)
    print("[Battle] BattleViewModel.handleSpineEvent. aniName: " .. aniName .. ", eName: " .. eName .. ", gameid: " .. gameid)
    ...
    elseif eName == "Hit" then      --动作加数字
        if aniName ~= "gj-1" and aniName ~= "zj" then
            return;
        end
...
function BattleViewModel.GetFightRewardNTF(buffer,tag)
    ...
    if tab.weekCopyNum then
        StormControl.SaveWeekBossTime(tab.weekCopyNum)
    end
    if StormViewModel.CurPointData.battleMap ~= "0" then
        ---更新数据统计
        TaskControl.ChangeStatistics(tab.day,tab.week,tab.month,tab.glory)
