...
function M:InitSound()
    ...
    --TODO
    self.Tog_AllSoundOn02().isOn = a == 1
    self.Tog_AllSoundOff02().isOn = a == 2

    Tools.ToggleValueChange(self.Tog_AllSoundOn02(),function(isOn)
        if isOn then
            SettingViewModel.SetAllSound(1)
        end
    end,nil)
    Tools.ToggleValueChange(self.Tog_AllSoundOff02(),function(isOn)
        if isOn then
...
function M:InitAgreement()
    ...
    UIEvent.LuaClick(self.Btn_Xiaochu().gameObject,function()
        MgrUI.Pop(UID.ConfirmPop_UI, {MgrLanguageData.GetLanguageByKey("ui_qita_text130"), function ()
            MgrSdk.FlyFunDelAcc(function(request)
                if request == "0" then
                    UnityEngine.DebugEx.LogError("[Debug] MgrSdk.FlyFunDelAcc calalback. success");
                    MgrSdk.isSwitchAccount = true
                    MgrSdk.BackToLogin()
                else
                    UnityEngine.DebugEx.LogError("[Debug] MgrSdk.FlyFunDelAcc calalback. fail. code: " .. request);
                end
            end)
        end, nil, 2}, true)
    end)
    ...
    UIEvent.LuaClick(self.Btn_DC().gameObject,function()
        Tools.OpenUrl("https://discord.gg/pb6nkqhYrE")
    end)

    UIEvent.LuaClick(self.Btn_FB().gameObject,function()
        Tools.OpenUrl("https://www.facebook.com/profile.php?id=61565193803516")
    end)
end
function M:InitCode()
...
function M:RefreshBindState()
    local type = MgrSdk.GetSdkUserAccountType()
    self.Btn_Bangding().gameObject:SetActive(type == 0)
    self.Btn_Bangding01().gameObject:SetActive(type == 0)
    self.Bangdingzhong_light().gameObject:SetActive(type == 2)
    self.Bangdingzhong_light01().gameObject:SetActive(type == 1)
    self.Bangdingzhong_grey().gameObject:SetActive(type ~= 2)
    self.Bangdingzhong_grey01().gameObject:SetActive(type ~= 1)
end
