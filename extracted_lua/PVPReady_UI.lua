...
function M:UpdataPlayerInfo()
    ...
    self.Text_Score().text = MgrLanguageData.GetLanguageByKey("pvppause_ui_score").."<color=#FFD075>"..self.RankData.score.."</color>"
    ...
    if self.RankData.score >= tonumber(SteamLocalData.tab[107014][2]) then
        ...
        self.Text_Bili().text = MgrLanguageData.GetLanguageByKey("ui_quickbattle_text14") .. ":<color=#FFCE48>"..tostring(self.RankData.RankLevelPercent / 100).."%</color>"
    else
    ...
    if self.RankData.score >= tonumber(SteamLocalData.tab[107014][2]) then
        ...
        if self.RankData.score >= tonumber(tab[nextIndex][4]) then
            self.Text_DisNextLv().text =string.format(MgrLanguageData.GetLanguageByKey("pvpready_ui_tips1"),self.RankData.rank - data[6])
        else
            self.Text_DisNextLv().text =string.format(MgrLanguageData.GetLanguageByKey("pvpready_ui_tips2"),tonumber(data[4]) - self.RankData.score)
        end
    end
end
...
function M:SaveFightTeam()
    ...
    if PVPViewModel.type == 0 then
        ---拷贝阵容到选中阵型
        TeamControl.ChangeTeamInfo(10000,fighters)
    ...
    elseif PVPViewModel.type == 1 then
        if self.PVPAttackOrDefend then
            TeamControl.ChangeTeamInfo(10001,fighters)
            TeamControl.SendSaveTeamData({10001},true,Handle(self,function()
                self:ReloadOrder(self.LineUpOrOrder == 2)
                self.isChange = false
            end))
        else
            TeamControl.ChangeTeamInfo(10002,fighters)
    ...
    if PVPViewModel.isSettle and PVPViewModel.isSettle == true then
        ...
        if curTime >= startTime and curTime <= endTime then
            ...
            if time >= 86400 then
                ...
                self.Text_RankEndTime().text =string.format(MgrLanguageData.GetLanguageByKey("pvpready_ui_rankendtime_hour"),day,hour)
            else
                local hour = math.floor(time / 3600)
                local min = math.floor((time % 3600) / 60)
                self.Text_RankEndTime().text =string.format(MgrLanguageData.GetLanguageByKey("pvpready_ui_rankendtime_min"),hour,min)
            end
        end
    end
end
...
function M:LoadTeamRole(teamIndex,saveTeam)
    ...
    ---如果队伍为空
    if #team.info == 0 and (teamIndex == 10000 or teamIndex == 10001 or teamIndex == 10002) then
        tNeedSave = true
...
function M:PvPBattleACK(buffer, tag)
    ...
    if tab.errNo~=0 then
        ...
        elseif tab.errNo == ServerErrorCode.HighLadderBattleEx_11631 then
            ...
            MgrUI.Pop(UID.PopTip_UI,{MgrLanguageData.GetLanguageByKey("pvpready_ui_tips7"),2},true)
        end
    end
end
...
function M:CheckTeam(teamIndex)
    ...
    for i, v in pairs(teamType) do
        ...
        ---如果队伍为空
        if #team.info == 0 and (v == 10001 or v == 10002) then
            local teamStr = string.split(SteamLocalData.tab[104018][2],",")
        ...
        if count > PVPViewModel.RoleLimit then
            ...
            --修改阵容阵型
            TeamControl.ChangeTeamInfo(v,newTeam.info)
        end
    end
