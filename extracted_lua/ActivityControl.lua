...
function ActivityControl.InitBuyGift()
    for i, v in ipairs(DirectlylibaoLocalData.tab) do
        local tShop = {}
        if string.find(v[3],'_') then
            local tList = string.split(v[3],',')
            for i, v in ipairs(tList) do
                table.insert(tShop,string.split(v,'_'))
            end
        end
        local tData = {
            id = v[1],
            actId = v[2],
            shopId = next(tShop)==nil and tonumber(v[3]) or tShop,
    ...
    ActivityControl.SendGiftList(GiftCfg, 1)
end
---获取礼包直购配置
...
---逐条获取蛋池详情
function ActivityControl.SendGiftList(_GiftCfg,_id)
    local tGiftCfg = {}
    for i, v in pairs(_GiftCfg) do
        if v.isShowBtn then
            table.insert(tGiftCfg,i)
        end
    end
    local tGiftData = _GiftCfg[tGiftCfg[_id]]
    local tShopId = type(tGiftData.shopId) == "table" and tonumber(tGiftData.shopId[1][2]) or tGiftData.shopId
    ActivityControl.GetDetail(tShopId, nil, function()
        if #tGiftCfg > _id then
            ActivityControl.SendGiftList(_GiftCfg,_id+1)
        end
    end)
end

function ActivityControl.GetDetail(_shopId, callBack, ackCallBack)
    ---序列化
    ...
    MgrNet.SendReq(MID.CLIENT_SKIN_RANDOM_REQ,bytes,0,nil,
            ...
            function(buffer,tag)
                ...
                --红点一天显示一次
                if Gift_BuyTimes[tCurActId] < Gift_TotalTimes[tCurActId] then
                    local tStr = PlayerControl.GetPlayerData().UID.."_ActGift"
                    local tDay = UnityEngine.PlayerPrefs.GetInt(tStr,0)
                    local tCreatDay = Global.GetCreateRoleDays()
                    if tDay ~= tCreatDay then
                        UnityEngine.PlayerPrefs.SetInt(tStr,tCreatDay)
                        RedDotControl.GetDotData("ActiveGift"):SetState(true)
                    end
                end
    end)
end
---当前礼包直购活动ID
function ActivityControl.SetCurGiftID(_actId)
    Gift_CurActID = _actId
end
function ActivityControl.GetCurGiftID()
    return Gift_CurActID
end
