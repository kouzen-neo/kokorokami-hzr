...
function M:OnUpdateUI(pData)
    self.m_storyReplayInfo = CheckTable(PlotViewModel.GetStoryReplayInfo(pData[1]))
    MgrRes.LoadSprite(self.ImgBg(), "PlotReview/Activity/Icon/" .. (self.m_storyReplayInfo.eventcover or "nil"))
    self.TextName().text = self.m_storyReplayInfo.name or "nil";
    local chapterInfo = ChapterLocalData.tab[CheckNumber(self.m_storyReplayInfo.chapterid)];
...
function M:RefreshState()
    if Global.IsStrNil(self.m_storyReplayInfo.resourceid) then
        Global.LogError("[PlotReviewItemActivity]剧情回顾数据resourceid不存在")
        return
    end
