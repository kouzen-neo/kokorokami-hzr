...
function M:OnShow(pData)
    ...
    if self.InBattle then
     ...
     if self.BattleRole.IsLeft then
        --if self.BattleRole.heroFlag ~= nil and self.BattleRole.heroFlag ~= 0 and self.BattleRole.heroFlag ~= false  then
         --   self.Text_Rank().text = self.BattleRole.heroCurSkill
         --else
            --if self.RoleData.heroFlag ~= nil and self.RoleData.heroFlag ~= 0 and self.RoleData.heroFlag ~= false  then
            --    self.Text_Rank().text = self.RoleData.heroCurSkill
            --else
                ---技能等级

                --if self.RoleData.heroFlag == 0 and self.RoleData:CheckHeroEquipIsMax() then
                    --self.Text_Rank().text = self.BattleRole.SkillLV + 1
                --else
    ...
    if self.InBattle then
        ...
        if self.BattleRole.Occupation == 4 then
            ...
            self.Text_Atk2().text = math.floor(self.RoleInfo.support / 10 + 0.5) / 10 .. "%"     --C#赋值问题，应该将辅助值赋给RealSuppart
        else
        ...
        if self.BattleRole.Occupation == 4 then
            MgrRes.LoadSprite(self.AtkIcon(), "Attribute/GearInfoIcon_2")
            self.Text_Atk1().text = MgrLanguageData.GetLanguageByKey("ui_yangcheng_text15")
            self.Text_Atk2().text = math.floor(self.BattleRole.RealSuppart * 1000 + 0.5) / 10 .. "%"
        else
            MgrRes.LoadSprite(self.AtkIcon(), "Attribute/GearInfoIcon_0")
            self.Text_Atk1().text = MgrLanguageData.GetLanguageByKey("ui_yangcheng_text14")
...
function M:CreatImage(id)
    ...
    Global.SetCoordinate(self.hd(), CharactercoordinatesLocalData.tab[id].coordinate13);
end
