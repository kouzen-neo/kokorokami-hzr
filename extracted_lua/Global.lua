...
function Global.GetLastMonthTotalDays()
    ...
    return tonumber(os.date("%d", os.time({year = nowDate.year, month = nowDate.month, day = 0})))
end
...
function Global.GetTimeFormat(tBeginTime,tEndTime)
    ...
    tEndTime[1].."/"..tEndTime[2].."/"..tEndTime[3].." "..tEndTime[4]..":"..tEndTime[5].." "..tUTC
end
...
function Global.GetAloneTimeFormat(_time)
    ...
    return tTime[1].."/"..tTime[2].."/"..tTime[3].." "..tTime[4]..":"..tTime[5].." "..tUTC
end
...
---@param num number
---@param digit int? @ 小数点后位数，默认 0，不可为负数或非整数(视为0)
---@return number @ digit 0 则返回整数
function math.round(num, digit)
    local times = 1
    if digit and math.type(digit) == "integer" and digit > 0 then
        times = math.pow(10, digit)
    end
    local res = math.floor(num * times + 0.5)
    -- 再执行除法会变成小数
    if times == 1 then
        return res
    end
...
---@param strKey string
---@return string
function Global.LanguageFormat(strKey, ...)
    return string.format(MgrLanguageData.GetLanguageByKey(strKey), ...)
end
