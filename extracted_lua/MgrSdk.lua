...
---该方法没用吗？设置界面中还在调用，且CMgrSdk中有该方法保留，FlyFunLogin则已经没有了。
    ...
    MgrSdk.CS:FlyFunCharge(roleId,roleName,roleLevel,
            ...
            function(request)
                ...
                local from = {
                    "userID="..roleId,
                    "orderID="..orderId,
                    "payResult="..(data[1]=="0" and 0 or 1),
                    "payResultCommit="..data[1],
                }
...
---美服埋点
function MgrSdk.JpTrackEvent(adjustEventCode)
    if not MgrSdk.IsFlyFun() then
        return
    end
    MgrSdk.CS:FlyFunTrackEvent(adjustEventCode)
end
...
function MgrSdk.BackToLogin()
    UnityEngine.DebugEx.LogError("[Debug] MgrSdk.BackToLogin.");
    MgrNet.IsSocket = false
    MgrNet.CacheMsg = {}
    if MgrUI.CurShowUIName == "Login_UI" then
        UnityEngine.DebugEx.LogError("[Debug] MgrSdk.BackToLogin. cancel: MgrUI.CurShowUIName == Login_UI");
        return
    end
    ...
    UnityEngine.DebugEx.LogError("[Debug] MgrSdk.BackToLogin. call MgrSce.Load. name: " .. MgrSce.Scenes.Home);
    MgrSce.Load(MgrSce.Scenes.Home, function ()
        UnityEngine.DebugEx.LogError("[Debug] MgrSdk.BackToLogin. MgrSce.Load callback. name: " .. MgrSce.Scenes.Home);
...
function MgrSdk.RequestReview()
    MgrSdk.CS:RequestReview()
end

function MgrSdk.ShowExitView()
    MgrUI.Pop(UID.ConfirmPop_UI,{MgrLanguageData.GetLanguageByKey("ui_sdk_showexit"), function()
        MgrSdk.QuitApp();
    end, nil, 1},true);
end
