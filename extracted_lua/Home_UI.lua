...
function M:OnUpdateUI()
    ...
    ---检查是否签到
    if not PlayerControl.GetPlayerData().sLoginSign or PlayerControl.GetPlayerData().monthSignFlag then
        PlayerControl.GetPlayerData().sLoginSign = true
        SignViewModel.Init()
    ...
    local skinData = HeroControl.GetSkinDataByRoleID(PlayerControl.GetPlayerData().HomeRole)
    if skinData then
        --检测ASMR
...
function M:RedDotFunc()
    ...
    for i, v in pairs(RedDotControl.GetDotData("Home").childNodeData) do
        ...
        elseif v.name == "PlotReview" then
            if v.State then
                self.PlotReviewRedDot().gameObject:SetActive(false)     -- 这个红点不要了
            else
...
function M:InitClick()
    ...
    UIEvent.LuaClick(self.Btn_Setting().gameObject, Handle(self, function()
        MgrUI.Pop(UID.SettingPop_UI, { { 7, 1, 2, 4, 6 }, function()
            HomeViewModel.isRoleVoice = true
        end }, true)
    end))
    ---跳转驾驶员
