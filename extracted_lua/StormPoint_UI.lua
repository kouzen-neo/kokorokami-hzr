...
function M:RefreshWeekBossTimes()
    if StormViewModel.CurScrollData.raidType == 13 then
        self.Text_Cishu().text = string.format(MgrLanguageData.GetLanguageByKey("ui_meizhoufuben_text2"),StormControl.GetWeekBossTime(),SteamLocalData.tab[125000][2])
...
--随机奖励转换格式
function M:RanRewardToString()
    local str = ""
    local data = StormViewModel.ResRewardData[StormViewModel.CurPointData.id].drop
    for i, v in ipairs(data) do
        str = str .. v.dropID .. "_" .. v.num
        if i < #data then
            str = str .. ","
        end
    end
