...
function M:ctor()
    ...
    self.CC = {
        -- Image 列表
        {'Img_bg','Img_bg',2},{'Img_Ziliaodi','Btn_Info/Img_Ziliaodi',2},{'Img_shutiao','Img_shutiao',2},{'Img_LVdi','Img_LVdi',2},{'RankIcon','RankIcon',2},{'Img_HeadMask','Img_HeadMask',2},{'HeadIcon','Img_HeadMask/HeadIcon',2},{'Img_HeadFrame','Img_HeadFrame',2},{'Img_Third','Img_Third',2},{'Img_Second','Img_Second',2},{'Img_First','Img_First',2},{'Img_Select','Img_Select',2},
        -- UITemplate 列表
        {'PVPRankItem','/',10},
        -- TextMeshProUGUI 列表
        {'Text_Ziliao','Btn_Info/Text_Ziliao',20},{'Text_Score','ScorePanel/Text_Score',20},{'Text_Number','ScorePanel/Text_Number',20},{'Text_RankName','Text_RankName',20},{'Text_LV','Img_LVdi/Text_LV',20},{'Text_Name','Text_Name',20},{'Text_Rank','Text_Rank',20},
    }
end
...
function M:CheckRank(isFirst)
    ...
    --    if self.rankInfo.score < 1000 then
    --        self.Text_RankName().text = MgrLanguageData.GetLanguageByKey("pvpready_ui_rank")
    --        MgrRes.LoadSprite(self.RankIcon(),"Level/liebingicon")
    --        break
    --    end
    --    if self.rankInfo.score >= tonumber(v[4]) then
    --        if self.rankInfo.score < tonumber(tab[1][4]) then
    --            self.Text_RankName().text = v[2]
    --            MgrRes.LoadSprite(self.RankIcon(),"Level/"..v[3])
    --            break
    --        else
    --            if self.rankInfo.rank + 1 >= v[5] and self.rankInfo.rank + 1 <=v[6] or isFirst then
    --                self.Text_RankName().text = v[2]
    --                MgrRes.LoadSprite(self.RankIcon(),"Level/"..v[3])
    --                break
    --            end
