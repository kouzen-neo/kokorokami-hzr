...
function M:InitMainPvP(type)
    ...
    if type == 1 then
        ...
        --local score = PlayerAvatarViewModel.BigHighLadderData.score
        --local rank = PlayerAvatarViewModel.BigHighLadderData.rank
        --for i, v in ipairs(SeniorPVPLocalData.tab) do
        --    if score == nil or score < 1000 then
        --        MgrRes.LoadSprite(self.RankIcon(),"Level/liebingicon")
        --        MgrRes.LoadSprite(self.Text_Rank(),"Level/liebing",nil,true)
        --        break
        --    end
        --    if score >= tonumber(v[4]) then
        --        if score < 2500 then
        --            MgrRes.LoadSprite(self.RankIcon(),"Level/"..v[3])
        --            MgrRes.LoadSprite(self.Text_Rank(),"Level/"..v[17],nil,true)
        --            break
        --        else
...
function M:ResetHeadItem(offset)
    local list = {}
    for i, v in ipairs(PlayheadLocalData.tab) do
...
function M:InitButton()
    ...
    UIEvent.LuaClick(self.Btn_Confirm().gameObject, Handle(self, function()
        ...
        if self.isNmae then
            ...
            ---判断是否包含特殊符号
            if Tools.ContainSpecialChar(name) or string.gmatch(name, "%S")() == nil then
