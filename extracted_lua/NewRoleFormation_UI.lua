...
function M:InitUpGrade()
    ...
    ---经验书物体
    self.ItemObj = {
        [1] = { self.Upitem1(), 110050, self.Img_Jiaobiao1 },
        [2] = { self.Upitem2(), 110051, self.Img_Jiaobiao2 },
        [3] = { self.Upitem3(), 110052, self.Img_Jiaobiao3 }
    }
...
function M:InitLvUpBtn()
    ...
    UIEvent.LuaClick(self.lvmindi().gameObject, Handle(self, Handle(self,self.MinLV)))
    UIEvent.LuaClick(self.Lvmaxdi().gameObject, Handle(self, Handle(self,self.MaxLV)))
    ---道具点击
...
function M:GetCurSumExp()
    local _ReturnExp = 0
    ---计算选择的经验书 经验总和
    _ReturnExp = _ReturnExp + (self.CurSelectItem.BookExp * self.CurSelectItem.BookSelectSum)
...
function M:ReceiveLevelUpNTF(buffer, tag)
    ...
    self:InitUpGrade()
    ---美服埋点-首次驾驶员升级
    if self.IsNoviceUp then
        MgrSdk.JpTrackEvent("727hql")
        self.IsNoviceUp = false
    end
end
---升级成功界面
