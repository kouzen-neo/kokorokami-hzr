...
function ShopItemData:GetResetBuyTime(time)
    ...
    if self.resetBuyType == 1 then
        ...
        --下次刷新时间
        local nextTime = rTime + 86400 - (tonumber(SteamLocalData.tab[112007][2]) - timeZone) * 3600
        --如果当前大于刷新时间
        if time >= rTime then
    ...
    elseif self.resetBuyType == 2 then
        ...
        --下次刷新时间
        local nextTime = os.time({year = targetTime.year, month = targetTime.month, day = targetTime.day, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3]) - (tonumber(SteamLocalData.tab[112007][2]) - timeZone) * 3600})
        --判断当天是否刷新日
        if day == weekNum then
            --如果当天刷新且当前时间已过刷新时间
            if time >= rTime then
...
function ShopItemData:IsShowRedDot()
    -- 特殊处理免费礼包
    if self.id == 119980 then
        if self.buyMaxCount - self.buyCount > 0 then
            return true
        end
    end
