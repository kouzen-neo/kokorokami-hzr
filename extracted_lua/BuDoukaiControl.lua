...
function BuDoukaiControl.OpenHome()
    ...
    if BuDoukaActivityInfo.systemopen == 0 or SysLockControl.CheckSysLock(BuDoukaActivityInfo.systemopen) and StormControl.CheckPointPass(tonumber(BuDoukaActivityInfo.front)) then
        MgrUI.GoHide(UID.WudaohuiPVP_UI)
    else
        MgrUI.Pop(UID.PopTip_UI,{SysLockControl.GetSystemLockTips(BuDoukaActivityInfo.systemopen),1},true)
    end
end
---布阵
