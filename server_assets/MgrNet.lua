...
function MgrNet.RegisterNTF(msgId, callFunc)
    UnityEngine.DebugEx.LogError("[QuickIOS] MgrNet.RegisterNTF. msgId: " .. msgId)
...
function MgrNet.ACKErroTips(msgId, buffer, tag)
    ...
    if MID.CLIENT_VERIFY_REQ == msgId or
       ...
       MID.CLIENT_SELECT_UP_GACHA_REQ == msgId
    then
    else
        if tab.errNo ~= 0 then
...
function MgrNet.GetServerTime()
    ...
    return MgrNet.CS:GetServerTime2()
end
...
function MgrNet.HttpGetOrderGenernal(userId,productId,callback)
    ---获取订单的http路径
    local get_url = "&url/GeneralOrder"
    ---配置表单
    local from = {
        "user_id="..userId,
        "product_id="..productId,
...
function MgrNet.JpLogin(userId, token, func)
    ---登录的http路径
    local login_url = "&url/httpLogin"
